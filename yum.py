...
def remove_plugin(name):
    plugin_directory = f"{working_dir}{sep}yumplugins{sep}"
    try:
...
async def download(name):
    html = await client.get(f'https://raw.githubusercontent.com/zkysimon/pagermaid_plugins/main/{name}/main.py')
...
@listener(is_plugin=False, outgoing=True, command="yum",
          need_admin=True,
          diagnostics=False,
...
async def plugin(__: Client, message: Message):
    ...
    if message.parameter[0] == "install":
        if len(message.parameter) == 1:
            ...
            failed_list = []
            no_need_list = []
            plugin_list = await client.get("https://raw.githubusercontent.com/zkysimon/pagermaid_plugins/main/list.json")
